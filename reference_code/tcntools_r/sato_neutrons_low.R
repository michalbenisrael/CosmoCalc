#' Compute low energy neutron flux using the analytical formulation of Sato et al. (2008)
#'
#'
#'
...
E = 10^seq(-8,0,length.out = 200)

# define structures to store results
flux=data.frame(matrix(NA, nrow=length(Rc), ncol=3))
colnames(flux)<-c("Rc","ethflux","thflux")
flux$Rc=Rc
#
...
ethflux = rep(0,length(Rc))
thflux = rep(0,length(Rc))
...
for(a in 1:length(Rc)) {
    ...
    # integration range for thermal and  epithermal neutrons
    ...
    clipindexeth = (E <= 1e-1) & (E >= 5e-7)
    clipindexth = (E <= 5e-7)

    flux$ethflux[a] = pracma::trapz(E[clipindexeth],PhiGMev[clipindexeth])
    flux$thflux[a] = pracma::trapz(E[clipindexth],PhiGMev[clipindexth])

} # end loop on a

res=list(E,PhiG_d,flux)
names(res) <- c("E","PhiG_d","flux")
